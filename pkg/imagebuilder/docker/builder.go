...
func (db Builder) Digest() (string, error) {
	reader, err := db.dockerBuilder.DockerClient.Client().ImageSave(db.Context, []string{db.Image})
	d, err := digest.Canonical.FromReader(reader)
	if err != nil {
		return "", err
	}
	defer reader.Close()
